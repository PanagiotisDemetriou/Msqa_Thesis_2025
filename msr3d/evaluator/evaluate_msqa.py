...
class MSQAEvaluator():
    ...
    #     return result_dict
    def eval_metrics(self):
        # Datasets to evaluate (from config)
        dataset_names_list = self.cfg.evaluate_dataset

        # Load full results JSON ONCE (top-level dict: {dataset_name: [instances...]})
        all_results = load_json(self.cfg.result_file)

        # Collect per-instance scored outputs per dataset
        result_scores_dict = {}

        for dataset_name in dataset_names_list:
            if dataset_name not in all_results:
                raise KeyError(
                    f"Dataset '{dataset_name}' not found in results file. "
                    f"Available keys: {list(all_results.keys())}"
                )

            dataset_instances = all_results[dataset_name]  # list of instance dicts
            score_list = []

            for i in tqdm(range(len(dataset_instances))):
                inst = dataset_instances[i]  # instance dict (DO NOT overwrite all_results)

                # Extract question
                if "question" in inst:
                    question = inst["question"]
                elif "instruction" in inst:
                    question = extract_question(inst["instruction"])
                else:
                    question = None  # or raise ValueError if you require it

                # Extract answers
                gt = inst["response_gt"][0]
                answer = inst["response_pred"]
                ...
                if self.cfg.gpt_score_flag:
                    score = self.evaluator.get_gpt_score(question, answer, gt)
                    scored_dict["gpt_score"] = (score - 1) * 25  # 1-5 -> 0-100

                # EM metrics
                lang_evaluator = EM_Evaluator()
                scored_dict.update(lang_evaluator.eval_instance(answer, [gt]))

                # Update running evaluator
                self.evaluator.update(scored_dict)

                # Type (if present)
                if "type" in inst:
                    scored_dict["type"] = inst["type"]
                else:
                    # If you want to enforce type existence, raise instead:
                    # raise ValueError("No type in instance.")
                    pass
        ...
        # Build stats dict dynamically for the datasets you are evaluating
        statistic_dict = {ds: {} for ds in dataset_names_list}

        metric_type_list = ["em1", "em1_strict"]
        if self.cfg.gpt_score_flag:
            metric_type_list.append("gpt_score")

        # Aggregate per dataset -> per metric -> per QA type
        for dataset_name in dataset_names_list:
            scores_data = result_scores_dict[dataset_name]

            for data_instance in scores_data:
                if "type" not in data_instance:
                    raise ValueError("No type in data_instance")
                data_QA_type = data_instance["type"].lower()

                for metric_type in metric_type_list:
                    statistic_dict[dataset_name].setdefault(metric_type, {})

                    for QA_type in QA_type_list:
                        if QA_type in data_QA_type:
                            statistic_dict[dataset_name][metric_type].setdefault(
                                QA_type, {"score": [], "cnt": 0, "avg": 0}
                            )
                            statistic_dict[dataset_name][metric_type][QA_type]["score"].append(
                                data_instance[metric_type]
                            )
                            statistic_dict[dataset_name][metric_type][QA_type]["cnt"] += 1

        # Compute averages per dataset/metric/type
        for dataset_name in dataset_names_list:
            for metric_type in metric_type_list:
                for QA_type in QA_type_list:
                    if (
                        metric_type in statistic_dict[dataset_name]
                        and QA_type in statistic_dict[dataset_name][metric_type]
                    ):
                        scores = statistic_dict[dataset_name][metric_type][QA_type]["score"]
                        cnt = statistic_dict[dataset_name][metric_type][QA_type]["cnt"]
                        statistic_dict[dataset_name][metric_type][QA_type]["avg"] = sum(scores) / cnt

        # Overall (weighted by counts) across datasets
        for metric_type in metric_type_list:
            ...
            for QA_type in QA_type_list:
                ...
                for dataset_name in dataset_names_list:
                    if (
                        ...
                        avg = statistic_dict[dataset_name][metric_type][QA_type]["avg"]
                        cnt = statistic_dict[dataset_name][metric_type][QA_type]["cnt"]
                        weighted_sum += avg * cnt
                        total_cnt += cnt

                if total_cnt > 0:
                    statistic_dict[metric_type]["overall"][QA_type] = weighted_sum / total_cnt
        ...
        for metric_type in metric_type_list:
            ...
            for QA_type in merged_QA_type_list:
                ...
                for dataset_name in dataset_names_list:
                    ds_metric = statistic_dict[dataset_name].get(metric_type, {})

                    if QA_type in ["counting", "existence", "navigation"]:
                        if QA_type in ds_metric:
                            weighted_sum += ds_metric[QA_type]["avg"] * ds_metric[QA_type]["cnt"]
                            total_cnt += ds_metric[QA_type]["cnt"]

                    elif QA_type == "attribute_description":
                        for sub in ["attribute", "description"]:
                            if sub in ds_metric:
                                weighted_sum += ds_metric[sub]["avg"] * ds_metric[sub]["cnt"]
                                total_cnt += ds_metric[sub]["cnt"]

                    elif QA_type == "spatial_refer":
                        for sub in ["spatial relationship", "refer"]:
                            if sub in ds_metric:
                                weighted_sum += ds_metric[sub]["avg"] * ds_metric[sub]["cnt"]
                                total_cnt += ds_metric[sub]["cnt"]

                    elif QA_type == "others":
                        for sub in ["affordance", "room type"]:
                            if sub in ds_metric:
                                weighted_sum += ds_metric[sub]["avg"] * ds_metric[sub]["cnt"]
                                total_cnt += ds_metric[sub]["cnt"]
                    else:
                ...
                if total_cnt > 0:
                    statistic_dict[metric_type]["merged"][QA_type] = weighted_sum / total_cnt
                    statistic_dict[metric_type]["merged"][QA_type + "_cnt"] = total_cnt

            # Weighted average across merged categories
            denom = sum(
                statistic_dict[metric_type]["merged"].get(QA + "_cnt", 0)
                for QA in merged_QA_type_list
            )
            if denom > 0:
                statistic_dict[metric_type]["merged"]["weighted_avg_score"] = sum(
                    statistic_dict[metric_type]["merged"][QA]
                    * statistic_dict[metric_type]["merged"][QA + "_cnt"]
                    for QA in merged_QA_type_list
                    if QA in statistic_dict[metric_type]["merged"]
                ) / denom

        # Build final output dict (same structure as your original intent)
        result_dict = {}

        # EM-R (em1)
        for key, val in statistic_dict["em1"]["merged"].items():
            if key.endswith("_cnt"):
                continue
            if key == "weighted_avg_score":
                result_dict["EM-R_overall"] = val
            else:
                result_dict[f"EM-R_{key}"] = val

        # GPT score (if enabled)
        if self.cfg.gpt_score_flag and "gpt_score" in statistic_dict:
            for key, val in statistic_dict["gpt_score"]["merged"].items():
                if key.endswith("_cnt"):
                    continue
                if key == "weighted_avg_score":
                    result_dict["GPT-Score_overall"] = val
                else:
                    result_dict[f"GPT-Score_{key}"] = val
