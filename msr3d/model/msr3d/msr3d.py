...
class MSR3D(nn.Module):
    def __init__(self, cfg):
        ...
        # >>> NEW: resolve their ids from THIS tokenizer (do not hardcode) <<<
        self.image_sp_token_id = self.llm_tokenizer.convert_tokens_to_ids(self.image_placeholder)
        self.scene_sp_token_id = self.llm_tokenizer.convert_tokens_to_ids(self.scene_placeholder)
        self.object_sp_token_id = self.llm_tokenizer.convert_tokens_to_ids(self.object_placeholder)
        ...
        #########
        print(f"Hidden Size:{self.llm_model.config.hidden_size}")
    ...
    def build_embeds(self,  
        ...
        """
        if img_sp_token is None:
            img_sp_token = self.image_sp_token_id
        if scene_sp_token is None:
            scene_sp_token = self.scene_sp_token_id
    ...
    def generate(
        ...
        try:
             ...
             traceback.print_exc()
             print("="*50, flush=True)
             print(f"source: {data_dict['source']}", flush=True)
             print(f"scan id: {data_dict['scan_id']}", flush=True)
             print(f"index: {data_dict['index']}", flush=True)
