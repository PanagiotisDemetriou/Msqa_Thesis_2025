...
model = dict(
    ...
    backbone=dict(
        ...
        pre_norm=True,
        enable_rpe=False,
        enable_flash=False,# Was True in PPT
        ...
        datasets=[
            ...
            dict(
                ...
                transform=[
                    ...
                    dict(type="RandomRotate", angle=[-1 / 64, 1 / 64], axis="x", p=0.5),# XXXX # 
                    dict(type="RandomRotate", angle=[-1 / 64, 1 / 64], axis="y", p=0.5),# XXXX # 
                    dict(type="RandomScale", scale=[0.9, 1.1]),
                    # dict(type="RandomShift", shift=[0.2, 0.2, 0.2]),
                    dict(type="RandomFlip", p=0.5),# XXXX # 
                    dict(type="RandomJitter", sigma=0.005, clip=0.02),
                    dict(
                        type="ElasticDistortion",
                        distortion_params=[[0.2, 0.4], [0.8, 1.6]],
                    ),# XXXX # 
                    dict(type="ChromaticAutoContrast", p=0.2, blend_factor=None),# XXXX # 
                    dict(type="ChromaticTranslation", p=0.95, ratio=0.05),# XXXX # 
                    dict(type="ChromaticJitter", p=0.95, std=0.05),
                    # dict(type="HueSaturationTranslation", hue_max=0.2, saturation_max=0.2),
                    # dict(type="RandomColorDrop", p=0.2, color_augment=0.0),
                    dict(
