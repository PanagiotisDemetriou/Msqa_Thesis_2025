...
def _load_pth_points_tuple(path):
    """
    Your .pth format: (xyz, rgb, a3, a4)
      xyz: (N,3) float32
      rgb: (N,3) float32
      a3 : (N,)
      a4 : (N,)
    Returns:
      xyz as (N,3) float32 numpy array
    """
    obj = torch.load(path, map_location="cpu")

    # torch.load may return tuple/list exactly as saved
    if isinstance(obj, (tuple, list)) and len(obj) >= 1:
        xyz = obj[0]
        xyz = np.asarray(xyz, dtype=np.float32)
        if xyz.ndim != 2 or xyz.shape[1] < 3:
            raise ValueError(f"First element in {path} is not Nx3. Got shape {xyz.shape}")
        return xyz[:, :3]

    # fallback: if it’s a raw array/tensor
    ...
    raise TypeError(f"Unsupported .pth payload type in {path}: {type(obj)}")
...
class PCATrans(object):
    ...
    def __call__(self, data):
        ...
        U, S, Vh = torch.linalg.svd(torch.t(pts), full_matrices=False)
        trans = U
...
class PointCloudDataset(Dataset):
    def __init__(self, root, mode=None, data_set='', data_list='', sparse_patches=False):
        ...
        if len(data_list) > 0:
            ...
            list_path = os.path.join(root, 'list', data_list + '.txt')
    ...
    def get_data(self, cur_sets):
        for s in tqdm(cur_sets, desc='Loading data'):
            pth_path = os.path.join(self.data_dir, f"{s}.pth")
            if not os.path.exists(pth_path):
                raise FileNotFoundError(f"Missing point cloud: {pth_path}")

            pcl = _load_pth_points_tuple(pth_path)  # (N,3) float32
            print(f"[{s}] pcl shape={pcl.shape}, dtype={pcl.dtype}, min={pcl.min():.3f}, max={pcl.max():.3f}")
